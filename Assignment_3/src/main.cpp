...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    if (intersect_sphere(ray_origin, ray_direction,sphere_center,sphere_radius)== false)
    {
        return -1;
    }
    else
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    double t = -1;
    if (intersect_parallelogram(ray_origin, ray_direction, pgram_origin, pgram_u, pgram_v) == false)
    {
        return -1;
    }
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    Vector3d p;
    Vector3d N;
    const int obj = find_nearest_object(light_position, -ray_direction, p, N);
    if (obj < 0){
        return true;
    }
    else if (p.isApprox(ray_origin,tolerance* 2)){
        return true;
    }
    else {
        return false;
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        // TODO: Shoot a shadow ray to determine if the light shrould affect the intersection point and call is_light_visible
        ...
        const double cos_theta = std::max(h.dot(N), 0.0);
        const double specular_intensity = pow(cos_theta, obj_specular_exponent);
    ...
    if (max_bounce != 0)
    {
        ...
        const Vector3d new_point  = p + tolerance * r;

        reflection_color = (refl_color.cwiseProduct(shoot_ray(new_point, r, max_bounce - 1)));
    }
...
void raytrace_scene()
{
    ...
    double image_y = tanf(field_of_view / 2) * focal_length; //TODO: compute the correct pixels size
    double image_x = tanf(field_of_view / 2) * focal_length * aspect_ratio; //TODO: compute the correct pixels size
